...
```{r}
disciplinas_p = c("1411174", "1411180", "1411167")
disciplinas_s = c("1109113", "1411168", "1411181", "1411170")
disciplinas_t = c("1411179", "1411176", "1411171", "1411172")

filtered_data = df_alunos_cc %>% filter(Cod_Disciplina %in% disciplinas_s & Media_Int == "55")
...
prob_reprovar_1nd_per_simples = 1.0 - cpquery(fittedbn, 
                                          event = (Situacao == "Aprovado"), 
                                          evidence = (Cod_Disciplina %in% disciplinas_p))

prob_reprovar_2nd_per_simples = 1.0 - cpquery(fittedbn, 
                                          event = (Situacao == "Aprovado"), 
                                          evidence = (Cod_Disciplina %in% disciplinas_s))


prob_reprovar_3nd_per_simples = 1.0 - cpquery(fittedbn, 
                                          event = (Situacao == "Aprovado"), 
                                          evidence = (Cod_Disciplina %in% disciplinas_t))
...
candidatos_p1_medias = df_alunos_cc %>% 
  filter(Cod_Disciplina %in% disciplinas_p) %>% 
...
safa_prob_aprovar_2nd_per = cpquery(fittedbn_safa, 
                                     event = (Situacao == "Aprovado" & 
                                              Cod_Disciplina %in% disciplinas_s), 
                                     evidence = (P1 == 100 & IC == 100))
